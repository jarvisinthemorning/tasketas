...
PROFILE_ASSUMPTIONS = {
    ...
    "tempest-revenant": [
        "Conditional on Spirit Swap; hero-selection, random Dark Gift, repeated Elemental fuel, and cycle-card acquisition odds are excluded.",
        "The assembly turn grants no loop. Later turns add one Air Revenant activation, distribute every active Easterly Winds randomly across six Tavern slots, and copy the slot with the most hits; base Tavern-minion stats are excluded.",
    ],
...
def _simulate_legacy(
    ...
    if profile == "tempest-revenant":
        ...
        for turn in range(online_turn, checkpoint_turn + 1):
            if turn == online_turn:
                traces.append(
                    _snapshot(
                        ...
                        [
                            (
                                "Engine assembled; the first full Revenant activation, refresh, Spirit Swap, "
                                "and three-Elemental loop is deferred to the next recruit turn"
                            )
                        ],
                    )
                )
            ...
            tavern_hits = [0] * SHOP_SLOTS[6]
            for _ in range(active_winds):
                tavern_hits[rng.randrange(len(tavern_hits))] += 1
            selected_anchor_hits = max(tavern_hits)
            wind_stats = 6 * selected_anchor_hits
            anchor_attack = swapped_attack + wind_stats
            anchor_health = wind_stats
            for tempest in tempests:
                multiplier = 2 if tempest["golden"] else 1
                tempest["attack"] += anchor_attack * multiplier
            ...
            traces.append(
                _snapshot(
                    ...
                    [
                        (
                            f"Resolved {active_winds} active Easterly Winds across random Tavern slots; "
                            f"the selected anchor received {selected_anchor_hits} hit(s), including "
                            f"+{wind_stats}/+{wind_stats}; "
